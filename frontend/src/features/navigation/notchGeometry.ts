...
export const NAV_GEOMETRY = {
   ...
   *
   * Es el número que decide qué tan "apoyado" se ve el círculo. Cuanto más chico, más abajo queda
   * y más hundido en la barra.
   */
  notchLift: 10,
   ...
   *
   * Tiene que ser el mismo que el padding horizontal de la barra, porque de ahí sale el centro de
   * cada sección. `MobileNav.tsx` usa esta constante para las dos cosas, así que no pueden
   * separarse.
   */
  trackInset: 12,
} as const
 ...
 * posición intermedia del hueco. Es la misma cuenta que hace el CSS para el círculo, así que los
 * dos caen siempre en el mismo lugar.
 */
 ...
 *
 * El recorrido, de izquierda a derecha: el labio de entrada, el arco grande del hueco, el labio
 * de salida, y el cierre por arriba de la barra, donde no hay material que sacar.
 */
export function notchCutPath(centerX: number): string {
  ...
  /*
    ...
    toca por afuera). Con esas dos condiciones, Pitágoras da la única distancia horizontal posible
    entre el centro del hueco y el del labio.
  */
  const lipDistance = Math.sqrt((R + r) ** 2 - (r + lift) ** 2)

  /*
    Donde el labio se encuentra con el arco grande. Está sobre la recta que une los dos centros, a
    `R` del centro del hueco: es la definición de tangencia entre dos círculos.
  */
